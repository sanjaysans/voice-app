...
type EvalRun = EvalRunSummary & {
  case_runs: Array<{
    ...
    metrics: Array<{ metric_key: string; score: number; explanation: string; judge_metadata: Record<string, unknown> }>;
  }>;
};

const emptyCaseForm = { caseName: "", initialUtterance: "", sampleResponse: "", outcome: "", requiredText: "" };
type SuiteTab = "overview" | "cases" | "history";
type EvaluationExecutionMode = "text_chat" | "live_audio";
...
export function EvaluationSuiteScreen({ suiteId }: { suiteId: string }) {
  ...
  const [executionMode, setExecutionMode] = useState<EvaluationExecutionMode>("text_chat");
    ...
    try {
      await runAction.run(async () => {
        const response = await api<EvalRun>(`${suitePath}/runs`, {
          method: "POST",
          body: JSON.stringify({ execution_mode: executionMode, repeat_count: 1 }),
        });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center justify-end gap-3"><div className="flex items-center gap-1 rounded-2xl border border-border bg-white p-1" role="tablist" aria-label="Evaluation execution mode"><button type="button" role="tab" aria-selected={executionMode === "text_chat"} onClick={() => setExecutionMode("text_chat")} className={`rounded-xl px-3 py-2 text-xs font-semibold ${executionMode === "text_chat" ? "bg-[rgba(102,89,255,0.12)] text-accent" : "text-[#6D6D78]"}`}>Text chat</button><button type="button" role="tab" aria-selected={executionMode === "live_audio"} onClick={() => setExecutionMode("live_audio")} className={`rounded-xl px-3 py-2 text-xs font-semibold ${executionMode === "live_audio" ? "bg-[rgba(102,89,255,0.12)] text-accent" : "text-[#6D6D78]"}`}>Live voice</button></div><Button variant="secondary" loading={refreshAction.isPending || isLoading || isHistoryLoading} loadingText="Refreshing" onClick={() => void refreshSuite()}><RefreshCw size={16} /> Refresh</Button><Button loading={runAction.isPending} loadingText="Running" onClick={() => void runSuite()} disabled={!suite.cases.length}><Play size={16} /> Run {executionMode === "text_chat" ? "text" : "voice"} suite</Button></div>
      </div>
