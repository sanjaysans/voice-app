...
export default function ConnectionsPage() {
  const { connections, toggleConnection, addConnection, updateConnection, deleteConnection } =
    useMockApp();
  ...
  const editingConnection = connections.find((connection) => connection.id === editingId) ?? null;

  const handleAddConnection = async () => {
    const nextName = label.trim() || `${vendor} ${category.toLowerCase()} connection`;
    setIsOpen(false);
    await addConnection({ category, vendor, name: nextName });
    setLabel("");
  ...
  return (
    <div className="space-y-6">
              ...
              <div key={connection.id} className="rounded-2xl border border-border bg-[#fcfcff] p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <Button variant="ghost" onClick={() => void deleteConnection(connection.id)}>
                      <Trash2 size={16} />
                      Delete
                    </Button>
                  </div>
      ...
      <Modal
        description="Refine the label or vendor without exposing real credentials in the prototype."
      ...
      >
        <div className="space-y-4">
          <Select
            ...
            options={["CRM", "Calendar", "Knowledge", "Telephony"]}
            onChange={(event) =>
              setCategory(event.target.value as "CRM" | "Calendar" | "Knowledge" | "Telephony")
            }
          />
          <Select
            label="Vendor"
            ...
            onChange={(event) => setVendor(event.target.value)}
          />
          <Input label="Connection label" value={label} onChange={(event) => setLabel(event.target.value)} />
          <Button className="w-full justify-center" onClick={() => void handleEditConnection()}>
            Save changes
          </Button>
        </div>
      </Modal>
