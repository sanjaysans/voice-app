...
def test_database_dsn_removes_sqlalchemy_driver_suffix() -> None:
    settings = Settings(database_url="postgresql+psycopg://voice:voice@localhost:5432/voice")
...
def test_database_dsn_leaves_plain_postgres_url_unchanged() -> None:
    settings = Settings(database_url="postgresql://voice:voice@localhost:5432/voice")
